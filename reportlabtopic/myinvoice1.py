...
class MyPrint:
    def __init__(self, buffer, pagesize, peldany):
        ...
        if peldany == '2':
           ...
           self.peldany = '3.1_Eredeti példánya'
    ...
    def _header_footer(canvas, doc, custom_footer):
        ...
        # Footer
        #footer = Paragraph('Footer' * 1, styles['Normal'])
    ...
    def print_invoice(self, vszamlaszam):
        ...
        doc = SimpleDocTemplate(buffer,
                                ...
                                encoding="utf-8",
                                fontName='DejaVuSans')
        # documentId = doc["id"].encode("utf-8")
        ...
        title.add(String(self.width/2+10,10, 'SZÁMLA', fontSize=14,fontName='DejaVuSans', fillColor=colors.black))
        ...
        #elements.append(Paragraph(selectedCeg.nev, styles['Normal']))
        # A large collection of style sheets pre-made for us
        styles = getSampleStyleSheet()
        #styles.add(ParagraphStyle(name='RightAlign', alignment=TA_RIGHT))
        # TO DO: image upload changes the name
        #elements.append(Image('raktarweb/static/images/raktar.png', hAlign= 'LEFT'))
        elements.append(Image(selectedCeg.logo, hAlign= 'LEFT'))

        #Add ceg data

        #logo = Image(selectedCeg.logo, styles["Normal"])
        ...
        vevorendszam = 'Gepjármű rendszáma:   ' + selectedSzamlatorzs.rendszam
        vevogyartmany = 'Gepjármű gyártmánya:   ' + selectedSzamlatorzs.gyartmany
        vevogepjarmutipus = 'Gepjármű tipusa:   ' + selectedSzamlatorzs.gepjarmutipus
        vevogepjarmufajta = 'Gepjarmű fajtája:   ' + selectedSzamlatorzs.gepjarmufajta
        vevogepjarmukmh = 'Gepjarmű km óra:   ' + selectedSzamlatorzs.gepjarmukmh

        cegadatok = [["Szolgáltató:", "Megrendelő:"],
                     [selectedCeg.nev, selectedVevo.nev],
                     [cegcim, vevocim],
                     [cegadoszam, vevorendszam],
                     [cegbankszamlaszam, vevogyartmany],
                     [cegemail, vevogepjarmutipus],
                     [cegweblap, vevogepjarmufajta],
                     ['', vevogepjarmukmh]]
        ...
        #proba kezdete
        val = 'Határidő'
        cegadatok = [['Megrendelő:', 'Megrendelő:', 'Fizetés', val],]
        adatok = [["Megrendelő:", "Számla kelte", "Teljesítés időpontja", "Fizetési határidő", "Számlaszám", "Rendelésszám"],]
        cegtable1 = Table(cegadatok, colWidths=[doc.width/6.0]*6)
        cegtable1.setStyle(TableStyle([('INNERGRID', (0, 0), (-1, -1), 0.25, colors.black),
                                        ('FONT', (1,0), (1,-1), 'DejaVuSans'),
                                        ('BOX', (0, 0), (-1, -1), 0.25, colors.black)]))
        elements.append(cegtable1)
        #proba vege
        ...
        #Add szamlaadatok
        szamlaadatok = [["Fizetés módja", "Számla kelte","Teljesítés időpontja",
                         "Fizetési határidő", "Számlaszám", "Rendelésszám"],
                        [selectedSzamlatorzs.fizetesimod,
                         selectedSzamlatorzs.szamlakelte.date().isoformat(),
                         selectedSzamlatorzs.teljesitesidopontja.date().isoformat(),
                         selectedSzamlatorzs.fizetesihatarido.date().isoformat(),
                         selectedSzamlatorzs.szamlaszam,
                         selectedSzamlatorzs.rendelesszam]]
